...
#calculate the world coordinates as a Pos entry
data modify storage dynagen: JunctionList append value {Pos:[0d,0d,0d]}
execute store result storage dynagen: JunctionList[-1].Pos[0] double 1 run scoreboard players operation #xOffset DGvar += #xCoord DGvar
execute store result storage dynagen: JunctionList[-1].Pos[1] double 1 run scoreboard players operation #yOffset DGvar += #yCoord DGvar
execute store result storage dynagen: JunctionList[-1].Pos[2] double 1 run scoreboard players operation #zOffset DGvar += #zCoord DGvar
...
execute store result storage dynagen: JunctionList[-1].Rotation int 1 run scoreboard players operation #rotation DGvar += #direction DGvar
